...
console.log('db', path.resolve(__dirname + '/../../db'));
...
var pageSock = shoe(function (stream) {
  var livestream = live(pages, { min: "", max: sep });
  livestream
...
var authorSock = shoe(function(stream) {
  var livestream = live(authors, { min: "", max: sep });
  livestream
    .pipe(new JSONStream({objectMode: true}))
    .pipe(stream);
...
var unpubPagesSock = shoe(function(stream) {
  ...
  stream.on('data', function(user) {
    ...
    livestream = live(unpubPages, {min: user + sep, max: user + sep + sep})
    livestream
      .pipe(new JSONStream({objectMode: true}))
      .pipe(stream);
  });
      ...
      fs.writeFile(outPath, string, function(err) {
        ...
        if (metadata.author) {
          ...
          if (validCheck(metadata)) {
            ...
            console.log('unpubPages', unpubKey);
            pages.del(pagesKey);

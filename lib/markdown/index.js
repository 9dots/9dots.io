...
app.use('/pages', function(req, res, next) {
  var key = namespace('views',path.basename(req.url, path.extname(req.url)));
...
function toHtml(srcPath) {
  ...
  fs.readFile(srcPath, function(err, data) {
    ...
    render(metadata, function(err, string) {
      ...
      mkdirp(path.basename(outPath), function(err) {
        if (err)
        ...
        fs.writeFile(outPath, string, function(err) {
          ...
          if (outPath.indexOf(__dirname + '/out/pages/') === 0) {
            var key = namespace('matter', path.basename(outPath, path.extname(outPath)));

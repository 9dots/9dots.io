...
.factory('setupUserRepo', ['editorAuth', 'editorModels', function(auth, models) {
  return function(owner, cb) {
    ...
    models[func](owner, auth.userRepo.name, {
        ...
        var server = config.backend;

        hook.config.url = server + '/post/webhook';
...
.factory('user', ['editorAuth', 'editorModels', 'editorEvents', function(auth, models, events) {
  ...
  //XXX this causes error on login
  getUserInfo();
...
.filter('avatar', ['users', function(users) {
  return function(user) {
    var def = "https://identicons.github.com/" + user + ".png";
    return "http://www.gravatar.com/avatar/" + ((users[user] && users[user].gravatar) || '') + "?s=29&d=" + def;
  };
}])
.run(['$rootScope', '$location', 'editorAuth', 'setupUserRepo', 'user', 'editorEvents', 'editorModels',
  function($rootScope, $location, auth, setupUserRepo, user, events, models) {

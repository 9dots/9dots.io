...
.factory('user', ['editorAuth', 'editorModels', function(auth, models) {
...
.factory('users', ['user', function(user) {
  ...
  user.on('loaded', function() {
    if (users[user.login] !== user.gravatar_id) {
...
.filter('avatar', ['users', function(users) {
  return function(user) {
    return "http://www.gravatar.com/avatar/" + users[user];
  }
}])
.run(['$rootScope', '$location', 'editorAuth', 'setupUserRepo', 'user',
  function($rootScope, $location, auth, setupUserRepo, user) {

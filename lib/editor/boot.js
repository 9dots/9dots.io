...
.factory('users', ['user', '$http', function(user, $http) {
  ...
  user.on('loaded', function() {
    console.log('user loaded', user.gravatar_id);
    if (user.gravatar_id && (!users[user.login] || users[user.login].gravatar !== user.gravatar_id)) {
      var userData = {id: user.login, gravatar: user.gravatar_id};
      console.log('put', user.login, SETTINGS.config.backend)

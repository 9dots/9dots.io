...
var queue = npm['queue-async'];
var store = require('store');
var Github = require('vendors').Github;
var url = require('url');
...
module.exports = {
  ...
  authenticated: function(auth, cb) {
    if (store.get('oauth-token')) 
  ...
  loadApplication: function(cb) {
    if (window.authenticated) {
      $.ajax({
        ...
        headers: {
          Authorization: 'token ' + store.get('oauth-token')
        },
        success: function (res) {
          store.set('avatar', res.avatar_url);
          store.set('username', res.login);
  ...
  createIfNotExist: function(username, repo, options, cb) {
    var user = github().getUser();
    user.userRepos(username, function(err, repos) {
      var foundRepo = _.find(repos, function(r) {
        return r.name === repo;
      });
      if (!foundRepo) {
        options = _.extend({name: repo}, options);
        user.createRepo(options, cb);
      } else 
        cb(null);
    });
  },

  orgCreateIfNotExist: function(org, repo, options, cb) {
    var user = github().getUser();
    user.orgRepos(org, function(err, repos) {
      ...
      if (!foundRepo) {
        options = _.extend({name: repo}, options);
        user.createOrgRepo(org, options, cb);
      } else 
        cb(null);
    });
  },

  createHook: function(user, repo, options, cb) {
    var repo = this.getRepo(user, repo);
    repo.createHook(options, cb);
  },

  createHookIfNotExist: function(user, repo, options, cb) {
    var repo = this.getRepo(user, repo);
    repo.listHooks(function(err, hooks) {
      var foundHook = _.find(hooks, function(hook) {
        return hook.config.url === options.config.url;
      });
      if (!foundHook)
        repo.createHook(options, cb);
      else
  ...
  isCollaborator: function(owner, repo, user, cb) {
    var repo = this.getRepo(owner, repo);
    repo.isCollaborator(user, cb);
  },
  ...
  _loadPostData: function(repo, path, file, cb, err, data, sha, xhr) {
    if (err) return cb(err);
...
//        'markdown': _.markdown(file),
  ...
  loadPost: function(user, repo, branch, path, file, cb) {
    ...
    repo.read(branch, path ? path + '/' + file : file, _.partial(
      this._loadPostData,
      repo,
      path,
      file,

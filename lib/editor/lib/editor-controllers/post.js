  ...
  function($scope, $routeParams, $element, userData, models, state, auth, events, pages, $location) {
    ...
    _.extend(this, {
      ...
      init: function() {
        ...
        //TODO: unbind events
        events.on('edit', _.bind(this.edit, this));
        events.on('preview', _.bind(this.preview, this));
        events.on('cancel', _.bind(this.cancel, this));
        events.on('save', _.bind(this.save, this));
        events.on('updateFile', _.bind(this.updateFile, this));
        events.on('deleteFile', _.bind(this.deleteFile, this));
        ...
        if (!this.newFile) {
          models.loadPost('9dots', repo.name, repo.branch, repo.path, file, _.bind(function(err, data) {
            if (err) 
              return this.notify('error', 'This file does not exist.');
            if (destroyed)
              return;

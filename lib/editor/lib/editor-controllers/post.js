        ...
        if (!this.newFile) {
          models.loadPost(user, repo.name, repo.branch, repo.path, fileWithExt, _.bind(function(err, data) {
            ...
            if (_.isString(this.data.metadata.objective)) {
              ...
              var that = this;
              var swbat = /student will|students will be able to|swbat/i;
              var objective = swbat.exec(this.data.metadata.objective);
              var idx = objective[0].length + objective.index;
              var sub = this.data.metadata.objective.substring(idx);
              this.data.metadata.objective = sub.split(',');
              console.log(this.data.metadata.objective);
              this.data.metadata.objective.forEach(function(obj, idx){
                obj = obj.replace(/^and/i, '');
                that.data.metadata.objective[idx] = capitaliseFirstLetter(obj.replace(/^[.\s]+|[.\s]+$/g, ''));
                console.log(that.data.metadata.objective);
              })
            }

  ...
  function($scope, $routeParams, $element, userData, models, state, auth, events, pages, $location) {
    ...
    _.extend(this, {
      ...
      init: function() {
        ...
        this.filepath = path.join(auth.repo.path, fileWithExt);
        console.log('file', fileWithExt);
        ...
        var ctrl = this;
        if (!this.newFile) {
          models.loadPost('9dots', repo.name, repo.branch, repo.path, fileWithExt, _.bind(function(err, data) {
            if (err)

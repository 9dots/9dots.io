...
var Ctrl = module.exports = 
  function($scope, $routeParams, $element, userData, models, state, auth, events, pages, $location, $templateCache, $http) {
        ...
        models.saveFile(this.user, repo.name, repo.branch, view.filepath, filecontent, message, function(err, res) {
          ...
          $scope.$safeApply(function() {
            ...
            function success() {
              ...
              view.stashFile(true);
              if (view.newFile) {
                $location.path('/edit/' + view.data.metadata.type + '/' + view.user + '/' + view.file);
...
Ctrl.$inject = ['$scope', '$routeParams', '$element', 'editorUserData', 'editorModels', 'editorState', "editorAuth", 'editorEvents', 'pages', '$location', '$templateCache', '$http']

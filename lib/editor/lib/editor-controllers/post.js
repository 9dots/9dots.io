  ...
  function($scope, $routeParams, $element, userData, models, state, auth, events, pages, $location, $templateCache, $http, $user, setupUserRepo, $sce) {
    ...
    _.extend(this, {
      ...
      init: function() {
        ...
        if (!this.newFile) {
          models.loadPost(user, repo.name, repo.branch, repo.path, fileWithExt, _.bind(function(err, data) {
            ...
            if(savedData)
              savedData.metadata.views = $scope.$pages['9dots-' + $scope.post.file].views;
            data.metadata.views = $scope.$pages['9dots-' + $scope.post.file].views;
            ...
            if (_.isString(this.data.metadata.objective)) {
              ...
              function objectiveToArray(objectives){
                var that = this;
                var swbat = /student will be able to|students will be able to|swbat/i;
                var objective = swbat.exec(objectives);
                if(!objective)
                  return [objectives];
                var idx = objective[0].length + objective.index;
                var sub = objectives.substring(idx);
                objectives = sub.split(',');
                objectives.forEach(function(obj, idx){
                  obj = obj.replace(/^\sand/i, '');
                  objectives[idx] = capitaliseFirstLetter(obj.replace(/^[.\s]+|[.\s]+$/g, ''));
                })
                return objectives;
              }

              this.data.metadata.objective = objectiveToArray(this.data.metadata.objective);
            }
      ...
      stashGet: function(save) {
        ...
        if(stash && this.data)
          stash.metadata.views = this.data.metadata.views;

  ...
  function($scope, $routeParams, $element, userData, models, state, auth, events, pages, $location, $templateCache) {
    ...
    _.extend(this, {
      ...
      init: function() {
        ...
        if (!this.newFile) {
          models.loadPost(user, repo.name, repo.branch, repo.path, fileWithExt, _.bind(function(err, data) {
            ...
            if (savedData && data && +new Date(data.metadata.update) < +new Date(savedData.metadata.update))
              this.data = savedData;
            else if (! err)
              this.data = data;
            else if (savedData) {
      ...
      stashFile: function(save) {
        ...
        if (filepath) {
          console.log('stash file', ns + filepath);
      ...
      computeDiff: function() {
        ...
        this.data.metadata.update = new Date();
      ...
      saveFile: function(filecontent, message) {
        var view = this;
        var repo = this.repo;
        models.saveFile(this.user, repo.name, repo.branch, view.filepath, filecontent, message, function(err, res) {
          console.log('res', res);
          ...
          $scope.$safeApply(function() {
            ...
            if (view.newFile) {
              $location.path('/edit/' + view.data.metadata.type + '/u/' + view.user + '/' + view.file);
            ...
            console.log('sendRender', view.user, view.fileWithExt, filecontent);
            markdown.sendRender(view.user, view.fileWithExt, filecontent);
          });
...
Ctrl.$inject = ['$scope', '$routeParams', '$element', 'editorUserData', 'editorModels', 'editorState', "editorAuth", 'editorEvents', 'pages', '$location', '$templateCache']
